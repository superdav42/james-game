...
- Decorative trees use layered green circles and a brown trunk. They have no gameplay effect.
...
- Reachable spyglass squares display board coordinates from `A1` to `J10`.
- Recent artillery targets use orange impact markers.
- A prominent team-colored label shows whose turn it is.
- Opposing units remain visually hidden throughout the active team's turn.

## Interaction

- All primary actions are touch-friendly: tap a unit, then tap a highlighted destination.
- Movement highlights include horizontal, vertical, and diagonal destinations.
